...
var modulesDescriptors = {
    ...
    creator: function(b, a) {
      ...
      $('.roll_kinghill', a).click(function() {
        ...
        function d() {
          ...
          h();
          console.log('kinghill_roll');
          return $.get('https://likes.fm/kinghill_roll', {
    ...
    creator: function(b, a) {
      ...
      $('.box_controls tbody', a).html(_.template($('#yesNoControlsTPL').html())).find('.button_blue button').text('Купить').click(function() {
        processPayment(Math.floor($('input.price', a).val() / 7 * 1000), function() {
          console.log('processPayment callback get_prepaid');
          return $.get('https://likes.fm/get_prepaid', function(c) {
            processPrepaid(c);
  ...
  activeOffers: _.extend({
    ...
    activatorClick: function(b, c) {
      var a = this;
      $.get('https://likes.fm/get_active_offers', function(d) {
    ...
    creator: function(b, a) {
      ...
      a.delegate('.line_cell .cancel', 'click', function(d) {
        ...
        var c = createPopup('Вы уверены, что хотите отменить этот заказ?', 'Заказ будет отменён и лайки будут возвращены на ваш баланс.<br>Вы сможете потратить их в любое время, заказав что-нибудь ещё.').find('.box_controls tbody').html(_.template($('#yesNoControlsTPL').html(), {yesNo: 1})).find('.button_blue button').click(function() {
          ...
          $.get('https://likes.fm/cancel_offer', {_id: e.attr('_id'), entity: e.attr('entity'), type: e.attr('type')}, function(g) {
            if (g.cmd) {
              var f = {
  ...
  referrals: _.extend({
    ...
    activatorClick: function(b, c) {
      var a = this;
      $.get('https://likes.fm/get_referrals', function(h) {
...
function proxy(b, c, a) {
  console.log('proxy',b, c, a);
  return $.ajax({
    url: 'https://likes.fm/proxy', data: b, global: a === undefined ? true : a, onError: function() {
      c('');
    }, success: function(d) {
      try {
...
function processPayment(f, d, a) {
  console.log('processPayment', f, d, a);
  var b = f;

  function c() {
    d().done(processPrepaid).fail(function() {
      var g = viewer_profile.prepaid;
      $.get('https://likes.fm/get_prepaid', function(h) {
        processPrepaid(h);
        clientException({
  ...
  if (f > 0 && !viewer_profile.is_admin) {
    if (false) {
      ...
      var e = setInterval(function() {
        $.ajax({
          url: 'https://likes.fm/check_payment', global: false, success: function(g) {
            if (!g) {
              return;
            }
            popup.onClose();
...
function initKinghillModule() {
  console.log('initKinghillModule');
...
$(function() {
  ...
  notifySound = new Sound('https://likes.fm/static/waterdrop.wav');
  ...
  $.ajaxSetup({
    cache: false, error: function(f, h, g) {
      ...
      if (g != 'abort' && _.include([0, 408, 504, 12029, 12030, 12031, 12152], f.status) && this.url.indexOf('retry=1') < 0 && this.url.indexOf('/save_offer') < 0 && this.url.indexOf('/get_offers') < 0 && this.url.indexOf('/get_state') < 0) {
        if (this.url.indexOf('https://likes.fm/app/') < 0) {
  ...
  $.when($.getJSON('https://api.vk.com/method/users.get?callback=?', {
    ...
    setInterval(function() {
      if (!isStatsModule() && !_.include(['kinghill'], $.bbq.getState('module'))) {
        $.ajax({
          url: 'https://likes.fm/get_state', global: false, success: function(h) {
            processPrepaid(h);
            $.getJSON('https://api.vk.com/method/users.get?callback=?', {
  ...
  $(document).delegate('a.compensationMenu', 'click', function(f) {
    console.log('compensationMenu click');
    if ($(this).hasClass('loading')) {
      return;
    }
    $(this).addClass('loading');
    $.post('https://likes.fm/accept_compensation', function(i) {
      var j = [], h = [];
      if (i.sub) {
        j.push('отписавшихся');
  ...
  $(document).delegate('.profile_actions .exit', 'click', function(f) {
    console.log('logout');
    $.ajax({
      url: 'https://likes.fm/logout', global: false, success: function(g) {
