...
(function($, s) {
    ...
    function w(M, K, L, J) {
        ...
        return this.each(function() {
            var P = $(this),
                N = K || k()[(this.nodeName || "").toLowerCase()] || "",
                O = N && P.attr(N) || "";
            P.attr(N, a[M](O, L, J))
        })
    }
...
(function() {
    ...
    b.getDayNumberFromName = function(j) {
        var q = d.dayNames,
            h = d.abbreviatedDayNames,
            p = d.shortestDayNames,
            l = j.toLowerCase();
    ...
    a.getUTCOffset = function() {
        var i = this.getTimezoneOffset() * -10 / 6,
            h;
        if (i < 0) {
            h = (i - 10000).toString();
...
(function() {
    ...
    var e = ("sunday monday tuesday wednesday thursday friday saturday").split(/\s/),
        ...
        p = ("Milliseconds Seconds Minutes Hours Date Week Month FullYear").split(/\s/),
        b = ("final first second third fourth fifth").split(/\s/),
        y;
    g.toObject = function() {
        var k = {};
    ...
    var c = function(i) {
        return function() {
            ...
            if (this._same) {
                ...
                var C = this.toObject(),
                    B = (arguments[0] || new Date()).toObject(),
                    A = "",
                    l = i.toLowerCase();
...
function Sound(b) {
    ...
    function b(g) {
        var f, i, j = "(?:,\\s*((?:1|0)(?:\\.0+)?|(?:0?\\.[0-9]+))\\s*)?\\)",
            h = new RegExp("rgb(a)?\\(\\s*([0-9]{1,3})\\s*,\\s*([0-9]{1,3})\\s*,\\s*([0-9]{1,3})\\s*" + j),
            k = new RegExp("rgb(a)?\\(\\s*([0-9]+(?:\\.[0-9]+)?)\\%\\s*,\\s*([0-9]+(?:\\.[0-9]+)?)\\%\\s*,\\s*([0-9]+(?:\\.[0-9]+)?)\\%\\s*" + j);
...
(function(g, h) {
    var i = "ui-state-active",
        ...
        d = k.END,
        l = g.browser.msie,
        c = g.browser.mozilla ? "DOMMouseScroll" : "mousewheel",
        o = ".uispinner",
        a = [m, n, s, f, j, p, q, d, k.BACKSPACE, k.DELETE, k.TAB],
        b;
    g.widget("ui.spinner", {
        options: {
            min: null,
            ...
            format: function(x, w) {
                var v = this,
                    y = /(\d+)(\d{3})/,
                    u = ((isNaN(x) ? 0 : Math.abs(x)).toFixed(w)) + "";
        ...
        _createButtons: function(F) {
            ...
            var I = this,
                ...
                O = y.width,
                z = y.showOn,
                D = g.support.boxModel,
                J = F.outerHeight(),
                S = I.oMargin = E(F.css("margin-right")),
                B = I.wrapper = F.css({
                    width: (I.oWidth = (D ? F.width() : F.outerWidth())) - O,
                    marginRight: S + O,
                    textAlign: "right"
                }).after('<span class="ui-spinner ui-widget"></span>').next(),
                M = I.btnContainer = g('<div class="ui-spinner-buttons"><div class="ui-spinner-up ui-spinner-button ui-state-default ui-corner-tr"><span class="ui-icon ' + y.upIconClass + '">&nbsp;</span></div><div class="ui-spinner-down ui-spinner-button ui-state-default ui-corner-br"><span class="ui-icon ' + y.downIconClass + '">&nbsp;</span></div></div>'),
                x, v, R, W, G, C, H, Q, K, P, A = F[0].dir == "rtl";
            if (w) {
                B.addClass(w)
            }
            B.append(M.css({
                height: J,
                left: -O - S,
                top: (F.offset().top - B.offset().top) + "px"
            }));
            R = I.buttons = M.find(".ui-spinner-button");
            R.css({
                width: O - (D ? R.outerWidth() - R.width() : 0),
                height: J / 2 - (D ? R.outerHeight() - R.height() : 0)
            });
            x = R[0];
            v = R[1];
            W = R.find(".ui-icon");
            W.css({
                marginLeft: (R.innerWidth() - W.width()) / 2,
                marginTop: (R.innerHeight() - W.height()) / 2
            });
            ...
            function N(aa, X) {
                ...
                var Z = String.fromCharCode(X || aa),
                    Y = I.options;
                if ((Z >= "0") && (Z <= "9") || (Z == "-")) {
                    return false
                }
...
(function(d) {
    ...
    function b(g, i, f) {
        var k = f.relative ? g.position().top : g.offset().top,
            j = f.relative ? g.position().left : g.offset().left,
            l = f.position[0];
    ...
    function a(h, j) {
        var s = this,
            ...
            q = 0,
            m = h.attr("title"),
            i = h.attr("data-tooltip"),
            t = c[j.effect],
            n, l = h.is(":input"),
            e = l && h.is(":checkbox, :radio, select, :button, :submit"),
            k = h.attr("type"),
            p = j.events[k] || j.events[l ? (e ? "widget" : "input") : "def"];
        if (!t) {
